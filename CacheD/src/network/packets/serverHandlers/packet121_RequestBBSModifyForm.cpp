...
{
// L2CacheD 0x0044ADE0
bool packet121_RequestBBSModifyForm(CIOSocket* pSocket, const uint8_t* packet)
{
    guard(L"bool serverHandlers::packet121_RequestBBSModifyForm(CIOSocket *pSocket, const uint8_t *packet)");
    ...
    if (user != NULL)
    {
        ...
        if (conn.Execute(L"EXEC lin2comm.dbo.lin_ReadArticle %d, %d, %d, %d, %d ", boardId, bId, user->GetAccountID(), user->GetPledgeID(), Config::s_worldId))
        {
            if (conn.Fetch())
            {
                ...
                nodeData = g_communityBoard.GetKeyword(boardId);
                if (nodeData)
                {
                    keywordCount = nodeData->TreeCount();
                    keywordBufLen = nodeData->TreeBufLen();
                    memcpy(keywordBuf, nodeData->TreeBuf(), treeBufLen);

                    serverSocket->Send(
                        "cdddSdbdbddddddSdSdSSddddSS",
                        CacheToServerPacket_BBSModifyForm,
                        unknown,
                        userId,
                        boardId,
                        name,
            ...
            {
                g_winlog.Add(LOG_ERROR, L"Error occcurred in RequestBBSModifyForm fetch");
            }
        }
        else
        {
            g_winlog.Add(LOG_ERROR, L"Error occurred in RequestBBSModifyForm");
        }
    }
    else
    {
