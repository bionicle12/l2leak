...
void Config::Load(const char* fileName)
{
        ...
        while (1)
        {
            ...
            else
            {
                switch (read_result)
                {
                    case Sym_Word:
                        if (state)
                        {
                            if (state == ParserState_ParameterFound)
                            {
                                ...
                                {
                                    g_winlog.AddLog(LOG_ERROR, "Insert twice %s at line %d in %s", paramName, lineNumber, fileName);
                                }
                            }
                            else if (state != ParserState_Comment)
                            {
                                g_winlog.AddLog(LOG_ERROR, "Invalid state at line %d in %s", lineNumber, fileName);
                            }
                        }
                        else
                        {
                    ...
                    default:
                        if (state != ParserState_Comment)
                            g_winlog.AddLog(LOG_ERROR, "Invalid token %s at line %d in %s", token, lineNumber, fileName);
                        break;
                }
            }
        }
    ...
    {
        g_winlog.AddLog(LOG_ERROR, "Can't load %s", fileName);
    }
...
Config::Symbol Config::GetToken(std::istrstream& stream, char* token, bool lookForWord)
{
    ...
    else if (Config::IsAlpha(symbol) || (symbol >= 128 && symbol < 254))  // ascii extended
    {
        ...
        if (!stream.eof())
        {
            ...
            if (stream.bad())
            {
                g_winlog.AddLog(LOG_ERROR, "putback");
            }
        }
        *token = '\0';
        result = Sym_Word;
    ...
    {
        g_winlog.AddLog(LOG_ERROR, "unexpected char '%c'", symbol);
        result = Sym_EoF;
    }
    ...
    if (result)
    {
        ...
        if (*newValue >= '0' && *newValue <= '9')
        {
            *valueToChange = atoi(newValue);
            g_winlog.AddLog(LOG_NORMAL, "Editor Mode, %s=%d\n", paramName, *valueToChange);
        }
    }
}
