#include "l2auth/utils/CoreDump.h"
...
void CoreDump::createReport(_EXCEPTION_POINTERS* ex)  // L2AuthD 0x004169E0
{
    ...
    if (::VirtualQuery((void*)contextEip, &memoryInfo, sizeof(memoryInfo)) && ::GetModuleFileNameA((HMODULE)memoryInfo.AllocationBase, moduleFilePath, sizeof(moduleFilePath)))
    {
    ...
    CoreDump::writeLine(hFile, "%s in module %s at %04x:%08x.\r\n", exceptionStr, moduleFileName, context->SegCs, contextEip);
...
void CoreDump::ImageHelpStackWalk(HANDLE hFile, CONTEXT* contextRecord)  // L2AuthD 0x00415DD8
{
    CoreDump::writeLine(hFile, "\r\nCall Stack Information\r\n");
    ::SymSetOptions(SYMOPT_DEFERRED_LOADS);

#if defined _WIN64
    LONGLONG contextEip = contextRecord->Rip;
    LONGLONG contextEsp = contextRecord->Rsp;
    LONGLONG contextEbp = contextRecord->Rbp;
#else
    LONG contextEip = contextRecord->Eip;
    LONG contextEsp = contextRecord->Esp;
    LONG contextEbp = contextRecord->Ebp;
#endif

    STACKFRAME stackFrame;
    memset(&stackFrame, 0, sizeof(stackFrame));
    stackFrame.AddrPC.Offset = contextEip;
    stackFrame.AddrPC.Mode = AddrModeFlat;
    stackFrame.AddrStack.Offset = contextEsp;
    stackFrame.AddrStack.Mode = AddrModeFlat;
    stackFrame.AddrFrame.Offset = contextEbp;
...
void CoreDump::IntelStackWalk(HANDLE hFile, CONTEXT* contextRecord)  // L2AuthD 0x004161BE
{
#if defined _WIN64
    LONGLONG contextEip = contextRecord->Rip;
    LONGLONG contextEbp = contextRecord->Rbp;
    ...
    const intptr_t* pcOffset = (const intptr_t*)contextEip;
    const intptr_t* addrFrameOffset = (const intptr_t*)contextEbp;
    do
    {
        char moduleFilename[MAX_PATH];
        moduleFilename[0] = 0;
        ...
        pcOffset = (intptr_t*)addrFrameOffset[1];
        const intptr_t* addrFrameOffsetPrev = addrFrameOffset;
        addrFrameOffset = (intptr_t*)*addrFrameOffset;  // FIXME: increment??
...
bool CoreDump::GetLogicalAddress(LPCVOID addr, LPSTR moduleName, DWORD nSize, int* section, int* offset)  // L2AuthD 0x00415CD1
{
    MEMORY_BASIC_INFORMATION memoryInfo;
    ...
    void* hMod = memoryInfo.AllocationBase;
    ...
    IMAGE_NT_HEADERS* pNtHdr = (IMAGE_NT_HEADERS*)((intptr_t)hMod + pDosHdr->e_lfanew);
    IMAGE_SECTION_HEADER* pSection = IMAGE_FIRST_SECTION(pNtHdr);

    intptr_t rva = (intptr_t)addr - (intptr_t)hMod;  // RVA is offset from module loadaddress
    int sectionIndex = 0;
...
void CoreDump::writeLine(HANDLE hFile, const char* format, ...)  // L2AuthD 0x0041612B
{
    va_list va;
    va_start(va, format);
    ...
    DWORD written;
    ::WriteFile(hFile, buffer, static_cast<DWORD>(len), &written, 0);
    va_end(va);
}

void CoreDump::writeModules(HANDLE hFile)  // L2AuthD 0x0041703B
{
    ...
    int pageCount = sizeof(intptr_t) * pagesPerGb;  // 4 GB / pageSize
    size_t pageIndex = 0;
...
void CoreDump::writeModuleInfo(HANDLE hFile, HMODULE hModule)  // L2AuthD 0x00417112
{
    char moduleFilename[MAX_PATH];
    ...
    if (moduleFile != INVALID_HANDLE_VALUE)
    {
        ...
        ::GetFileSizeEx(moduleFile, &fileSizeLarge);
...
void CoreDump::writeExtraInfo(HANDLE hFile)  // L2AuthD 0x004173B0
{
    ...
    DWORDLONG physMemory = memoryStatus.ullTotalPhys;
    DWORDLONG mbytes = physMemory >> 20;
#else
    MEMORYSTATUS memoryStatus;
    memoryStatus.dwLength = sizeof(MEMORYSTATUS);
    ::GlobalMemoryStatus(&memoryStatus);

    // CoreDump::writeLine(hFile, "%d MBytes physical memory.\r\n", (memoryStatus.dwTotalPhys + 0xFFFFF) >> 20);
    // FIXED:
...
void CoreDump::getFileTime(char* out, FILETIME fileTime)  // L2AuthD 0x004172E8
{
...
const char* CoreDump::exceptionToString(int exceptionCode)  // L2AuthD 0x004174C8
{
