...
#else
    ::SetFilePointer(hFile, FILE_BEGIN, 0, FILE_END);
#endif
...
bool CExceptionInit::sendMail(const char* ipAddress, const char* mailFrom, const char* mailTo, const char* errorLogPath)
{
    ...
    if (ipAddress[0] < '0' || '9' < ipAddress[0])  // FIXME: isalpha/isnum
    {
    ...
    static char MAIL_TEMPLATE[] = "Subject: AuthD crashed";
    rcvResult = ::recv(socket, buf, sizeof(buf), 0);
    int templateLen = ::strlen(MAIL_TEMPLATE);
    if (::send(socket, MAIL_TEMPLATE, templateLen, 0) <= 0)
    {
        ::closesocket(socket);
        ::WSACleanup();
        return false;
