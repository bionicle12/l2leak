...
#include "l2auth/db/SqlConnection.h"
...
void DBEnv::Init(int dbConnectionNumber)
{
    ...
    {
        g_winlog.AddLog(LOG_ERROR, "db env allocation failed");
        return;
    }
    ...
    g_winlog.AddLog(LOG_ERROR, "db login failed");
...
bool DBEnv::Login(bool reconnect)
{
    ...
    while (true)
    {
        ...
        sqlResult = ::SQLDriverConnect(dbcHandler, NULL, (SQLCHAR*)connectionStr, static_cast<SQLSMALLINT>(len), connStrOut, sizeof(connStrOut), &connStrOutLen, SQL_DRIVER_NOPROMPT);
...
void DBEnv::AllocSQLPool()
{
    for (int i = 0; i < connectionNumber; ++i)
    {
        ...
        if (sqlResult != SQL_SUCCESS && sqlResult != SQL_SUCCESS_WITH_INFO)
        {
            g_winlog.AddLog(LOG_ERROR, "hdbc allocation failed");
        ...
        else
        {
            ...
            sqlResult = ::SQLDriverConnect(sqlConnections[i].dbcSqlHandler, NULL, (SQLCHAR*)connectionStr, len, szConnStrOut, 1024, &pcchConnStrOut, SQL_DRIVER_NOPROMPT);

            if (sqlResult != SQL_SUCCESS && sqlResult != SQL_SUCCESS_WITH_INFO)
            {
                g_winlog.AddLog(LOG_ERROR, "hdbc connection failed");
            ...
            else
            {
                ...
                if (sqlResult != SQL_SUCCESS && sqlResult != SQL_SUCCESS_WITH_INFO)
                {
                    g_winlog.AddLog(LOG_ERROR, "stmt allocation failed");
                    ::SQLFreeHandle(SQL_HANDLE_DBC, sqlConnections[i].dbcSqlHandler);
                    sqlConnections[i].stmtSqlHandler = SQL_NULL_HANDLE;
                    sqlConnections[i].dbcSqlHandler = SQL_NULL_HANDLE;
                    sqlConnections[i].free = false;
...
void DBEnv::OnTimerCallback()
{
    if (Login(true))
    {
        ...
        else
        {
            ...
            if (setEnvRes != SQL_SUCCESS && setEnvRes != SQL_SUCCESS_WITH_INFO)
            {
                ...
                g_winlog.AddLog(LOG_ERROR, "db recovery failed: env allocation");
            }
            else
            {
                AllocSQLPool();
    ...
    {
        g_winlog.AddLog(LOG_ERROR, "db recovery failed: login impossible");
        RegisterTimer(30000, 1);
    }
    return ReleaseRef();
}
