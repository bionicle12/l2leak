...
bool CDBConn::Fetch(bool* notFound)
{
    ...
    if (sqlreturn != SQL_SUCCESS && sqlreturn != SQL_SUCCESS_WITH_INFO)
    {
        ...
        {
            g_winlog.AddLog(LOG_ERROR, "Fetch error = %d", sqlreturn);
        }
        return false;
    }
...
void CDBConn::Error(SQLSMALLINT handlerType, SQLHANDLE handler, const char* sqlQuery)
{
    ...
    if (sqlResult == SQL_SUCCESS)
    {
        if (sqlQuery != NULL)
        {
            g_winlog.AddLog(LOG_ERROR, "sql: %s", sqlQuery);
        }
        g_winlog.AddLog(LOG_ERROR, "%s:%s", sqlState, errorText);
...
//            if ( sqlQuery )
//            {
//                g_winlog.Add(LOG_ERROR, format, sqlQuery);
//            }

//            sqlResult = (int)g_winlog.Add(LOG_ERROR, "%s:%s", Sqlstate, errorText);
//        }
//    }
//}
...
SQLRETURN CDBConn::bindParam(SQLUSMALLINT index, SQLSMALLINT fParamType, SQLSMALLINT fCType, SQLSMALLINT fSqlType, SQLUINTEGER cbColDef, SQLSMALLINT ibScale, SQLPOINTER rgbValue, size_t cbValueMax, SQLLEN* pcbValue)
{
    return ::SQLBindParameter(m_sqlHandler, index, fParamType, fCType, fSqlType, cbColDef, ibScale, rgbValue, static_cast<SQLINTEGER>(cbValueMax), pcbValue);
}
