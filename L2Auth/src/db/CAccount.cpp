...
// L2AuthD 0x00401088
size_t CAccount::MakeBlockInfo(char* reasonBuffer)
{
    int blockCount = 0;
    ...
    if (sql.Execute(" Select reason, msg From block_msg with (nolock) Where uid = %d", uid))
    {
        ...
        while (sql.Fetch(&notFound))
        {
            ...
            wchar_t msg[256];
            // size_t msgLength = 2 * swprintf(msg, L"%s", blockMsg) + 2; FIXED
            size_t msgLength = 2 * swprintf(msg, sizeof(msg) / sizeof(*msg), L"%s", blockMsg) + 2;
    ...
    size_t length = iterator - reasonBuffer;
...
LoginFailReason CAccount::CheckNewPassword(const char* accName, char* realPass)
{
    ...
    size_t accLength = ::strlen(accName);
    SQLLEN len = SQL_NTS;
    ...
    size_t realPassLength = ::strlen(realPassCopy);
