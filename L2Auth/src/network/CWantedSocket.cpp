...
{
    g_winlog.AddLog(LOG_ERROR, "WantedSocket Deleted");
}
...
void CWantedSocket::OnClose()
{
    ...
    g_winlog.AddLog(LOG_ERROR, "*close connection WantedSocket from %s, %x(%x)", IP(), m_hSocket, this);
    AddRef();
...
void CWantedSocket::OnRead()
{
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            m_packetSize = buffer[dwRead] + (buffer[dwRead + 1] << 8) + 1;  // FIXME: - g_Config.PacketSizeType; missed
        ...
        // FIXME: initialized with IOCP::g_hCompletionPortExtra insie OnTimeout
        // and on WinMain : g_SocketWanted->Initialize(IOCP::g_hCompletionPortExtra)
