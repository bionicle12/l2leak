...
IPSocket::~IPSocket()
{
    g_winlog.AddLog(LOG_ERROR, "IPSocket Deleted");
}
...
void IPSocket::OnClose()
{
    ...
    g_winlog.AddLog(LOG_ERROR, "*close connection IPServer from %s, %x(%x)", IP(), m_hSocket, this);
...
bool IPSocket::packet03_handler(IPSocket* /*ipSocket*/, uint8_t* buffer)
{
    ...
    if (success)
    {
        ...
        if (authSocket != NULL)
        {
            ...
            {
                time_t loginTime = std::time(NULL);
                in_addr addr;
