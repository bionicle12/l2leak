...
void CSocketInt::packet01_changeSocketLimit(CSocketInt* socketInt, const char* command)
{
    ...
    if (token != NULL)
    {
        ...
        if (socketLimit > 0 && socketLimit < 12000)
        {
            ...
            g_winlog.AddLog(LOG_NORMAL, "Change socketlimit, %d", socketLimit);
            socketInt->Send("1");
        }
    }
}
...
void CSocketInt::packet04_changeGmMode(CSocketInt* socketInt, const char* command)
{
    ...
    if (token != NULL)
    {
        ...
        if (buffer[0] == '0')
        {
            ...
            g_winlog.AddLog(LOG_NORMAL, "GM ONLY MODE OFF");
            socketInt->Send("1");
        }
        else
        {
            g_Config.GMCheckMode = true;
            g_winlog.AddLog(LOG_NORMAL, "GM ONLY MODE ON");
            socketInt->Send("1");
        }
    }
    else
...
void CSocketInt::packet06_changeGmIp(CSocketInt* socketInt, const char* command)
{
    ...
    if (token != NULL)
    {
        ...
        else
        {
            ...
            g_winlog.AddLog(LOG_NORMAL, "Change GMIP, %d.%d.%d.%d", addr.S_un.S_un_b.s_b1, addr.S_un.S_un_b.s_b2, addr.S_un.S_un_b.s_b3, addr.S_un.S_un_b.s_b4);
...
void CSocketInt::packet08_changeServerMode(CSocketInt* socketInt, const char* command)
{
    ...
    if (buffer[0] == '0')
    {
        ...
        g_winlog.AddLog(LOG_NORMAL, "Config FreeServer is chaged to false");
        socketInt->Send("1");
        return;
    }
    ...
    if (buffer[0] == '1')
    {
        ...
        g_winlog.AddLog(LOG_NORMAL, "Config FreeServer is changed to true");
        socketInt->Send("1");
        return;
    }
...
void CSocketInt::packet11_kickUserFromWorldServer(CSocketInt* socketInt, const char* command)
{
    ...
    {
        WorldSrvServer::SendSocket(worldServer.ipAddress, "cdcs", AuthToServerPacket_KickAccountRequest, userInfoTable.uid, 7, accName);
        socketInt->Send("%d", 0);
    }
    else
    {
