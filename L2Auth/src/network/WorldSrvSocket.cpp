...
#include "l2auth/db/CServerUserCountStatus.h"
...
const WorldSrvSocket::PacketHandler WorldSrvSocket::handlers[WorldSrvSocket::HANDLERS_NUMBER] = {&WorldSrvSocket::packet00_playOk,
                                                                                                 ...
                                                                                                 &WorldSrvSocket::packet02_userLoggedToGame,
                                                                                                 &WorldSrvSocket::packet03_userQuitGame,
                                                                                                 ...
                                                                                                 &WorldSrvSocket::packet10_serverConnectedReply,
                                                                                                 &WorldSrvSocket::packet11_ReconnectedPlayerList,
...
void WorldSrvSocket::OnClose()
{
    ...
    g_winlog.AddLog(LOG_ERROR, "*close connection from %s, %x(%x)", IP(), m_socket, this);
...
void WorldSrvSocket::OnCreate()
{
    AddRef();
    OnRead();
    Send("cdd", AuthToServerPacket_AuthInitPacket, g_buildNumber, 1);
}
...
void WorldSrvSocket::OnRead()
{
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            {
                g_winlog.AddLog(LOG_ERROR, "%d: bad packet size %d", m_socket, m_packetSize);
                errorNum = 1;
                g_errLog.AddLog(
                    LOG_INF,
...
void WorldSrvSocket::Send(const char* format, ...)
{
    ...
    if (m_socketStatus != SocketStatus_Closed)
    {
        ...
        {
            g_winlog.AddLog(LOG_ERROR, "%d: assemble too large packet. format %s", m_socket, format);
        }
...
{
    uint32_t uid = BufferReader::ReadInt(&buffer);
    ...
    if (socket != 0 && uid != 0 && otherServer != 0)
    {
        ...
        if (otherServer > 0 && otherServer <= g_serverList.m_serverNumber)
        {
            ...
            WorldSrvServer::SendSocket(worldServer.ipAddress, "cdcs", AuthToServerPacket_KickAccountRequest, uid, 14, accountName);
        }
        worldSrvSocket->Send("cdcs", AuthToServerPacket_KickAccountRequest, uid, 14, accountName);
        return false;
    }

    if (socket != 0 && uid != 0)
    {
        uint8_t serverId = static_cast<uint8_t>(worldSrvSocket->m_serverID);
        CAuthSocket::Send(socket, "cddc", AuthToClientPacket_PlayOk, sessionKey, uid, serverId);
    }
...
bool WorldSrvSocket::packet01_playFail(WorldSrvSocket* worldSrvSocket, uint8_t* buffer)
{
    ...
    {
        worldSrvSocket->Send("cdcs", AuthToServerPacket_KickAccountRequest, uid, errorCode, accName);
    }

    if (socket != 0 && uid != 0)
    {
        CAuthSocket::Send(socket, "cc", AuthToClientPacket_PlayFail, errorCode);
    }
...
// L2AuthD 0x0040F8B2
bool WorldSrvSocket::packet02_userLoggedToGame(WorldSrvSocket* worldSrvSocket, uint8_t* buffer)
{
    int uid = BufferReader::ReadInt(&buffer);
...
// L2AuthD 0x0040F900
bool WorldSrvSocket::packet03_userQuitGame(WorldSrvSocket* worldSrvSocket, uint8_t* buffer)
{
    int uid = BufferReader::ReadInt(&buffer);
    ...
    {
        g_errLog.AddLog(LOG_ERROR, "quit game error, uid %d, reason %d, usetime %d\r\n", uid, reason, usetime);
    }
...
bool WorldSrvSocket::packet08_pingAck(WorldSrvSocket* worldSrvSocket, uint8_t* buffer)
{
    ...
    worldSrvSocket->Send("cd", AuthToServerPacket_PingPacket, ping);
    return false;
}
...
// L2AuthD 0x0040FC7A
bool WorldSrvSocket::packet10_serverConnectedReply(WorldSrvSocket* worldSrvSocket, uint8_t* /*buffer*/)
{
...
// L2AuthD 0x0040FD0D
bool WorldSrvSocket::packet11_ReconnectedPlayerList(WorldSrvSocket* worldSrvSocket, uint8_t* buffer)
{
    ...
    for (userIndex = 0; userIndex < userNumber; ++userIndex)
    {
        ...
        accInfo->loginTime = std::time(NULL);
    ...
    worldSrvSocket->Send("cd", AuthToServerPacket_ReconnectReply, userIndex);
...
bool WorldSrvSocket::packet12_newCharCreated(WorldSrvSocket* worldSrvSocket, uint8_t* buffer)
{
    ...
    SOCKET socket = g_accountDb.FindSocket(uid, false);
    CAuthSocket::Send(socket, "ccddchb", AuthToClientPacket_CharSelection, unknown, uid, charId, worldSrvSocket->m_serverID, binarySize, binarySize, buffer);
...
bool WorldSrvSocket::packet13_charDeleted(WorldSrvSocket* worldSrvSocket, uint8_t* buffer)
{
    ...
    SOCKET socket = g_accountDb.FindSocket(uid, false);
    CAuthSocket::Send(socket, "ccddc", AuthToClientPacket_CharDeleted, unknown, uid, charId, worldSrvSocket->m_serverID);
...
bool WorldSrvSocket::packet14_charManipulation(WorldSrvSocket* worldSrvSocket, uint8_t* buffer)
{
    ...
    if (unknown > 0)
    {
        ...
        CAuthSocket::Send(authSocket, "cdccchb", AuthToClientPacket_CharManipulation, uid, 11, unknown, worldSrvSocket->m_serverID, binarySize, binarySize, buffer);
    }
...
bool WorldSrvSocket::packet15_serverSelected(WorldSrvSocket* /*worldSrvSocket*/, uint8_t* buffer)
{
    ...
    if (serverId >= 1 && serverId <= g_serverList.GetMaxServerNum())
    {
        ...
        {
            CAuthSocket::Send(authSocket, "cc", AuthToClientPacket_PlayFail, PLAY_FAIL_SOME_ERROR);
        }
        else
        {
            CAuthSocket::Send(authSocket, "cddd", AuthToClientPacket_ServerSelection, unknown, worldServer.outerIP, worldServer.serverPort);
        }
    }
    else
    {
...
bool WorldSrvSocket::packet18_updateGSStatus(WorldSrvSocket* /*worldSrvSocket*/, uint8_t* buffer)
{
    ...
    if (serverId > 0 && serverId <= g_serverList.m_serverNumber)
    {
        ...
        if (worldServer.serverId > 0 && worldServer.serverId <= g_serverList.m_serverNumber)
        {
            ...
            {
                g_winlog.AddLog(LOG_ERROR, "%d serverid is manage servers (not allow protocol)", worldServer.serverId);
            }
        }
    }
...
bool WorldSrvSocket::packet20_getServerList(WorldSrvSocket* worldSrvSocket, uint8_t* /*buffer*/)
{
    ...
    packetBuffer[size] = 0;
    worldSrvSocket->Send("cb", AuthToServerPacket_ServerList_other, size, packetBuffer);
