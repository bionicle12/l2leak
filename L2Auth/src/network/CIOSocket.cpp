...
{
#if defined _WIN64
    SOCKET oldSocket = (SOCKET)::InterlockedExchange64((LONGLONG*)(&m_hSocket), INVALID_SOCKET);
#else
    SOCKET oldSocket = (SOCKET)::InterlockedExchange((LONG*)(&m_hSocket), INVALID_SOCKET);
#endif
...
void CIOSocket::Initialize(HANDLE existingCompletionPort)
{
    ...
    {
        g_winlog.AddLog(LOG_ERROR, "Initilize CompletionPort Error CloseSocket");
        CloseSocket();  // FIXED
    }
}
...
void CIOSocket::OnReadCallback(int dwTransferred)
{
    if (dwTransferred != 0)
    {
        ...
        {
            g_winlog.AddLog(LOG_ERROR, "Invalid Socket!");
        }
    }
    else
    {
...
void CIOSocket::OnWriteCallback(int dwTransferred)
{
    ...
    if (dwTransferred == m_pFirstBuf->m_dwSize)
    {
        ...
        if (m_pFirstBuf)
        {
            ...
            if (::WSASend(m_hSocket, &buffers, 1u, &bytesSent, 0, &m_overlappedWrite, 0) && ::GetLastError() != ERROR_IO_PENDING)
            {
                ...
                {
                    g_winlog.AddLog(LOG_ERROR, "CIOSocket::WriteCallback %x(%x) err=%d", m_hSocket, this, errorCode);
                }
                ReleaseRef();
            }
        }
    ...
    {
        g_winlog.AddLog(LOG_ERROR, "different write count %x(%x) %d != %d", m_hSocket, this, dwTransferred, m_pFirstBuf->m_dwSize);
...
void CIOSocket::Read(size_t dwLeft)
{
    ...
    if (result != ERROR_SUCCESS)
    {
        ...
        if (result != ERROR_IO_PENDING)
        {
            ...
            if (errorCode != WSAENOTSOCK && errorCode != WSAECONNRESET && errorCode != WSAECONNABORTED)
            {
                g_winlog.AddLog(LOG_ERROR, "CIOSocket::Read %x(%x) err = %d", m_hSocket, this, errorCode);
            }
            CloseSocket();
            ReleaseRef();
        }
    ...
    if (buff->m_dwSize)
    {
        ...
        else
        {
            ...
            if (::WSASend(m_hSocket, &wsabuf, 1u, &dwSent, 0, &m_overlappedWrite, 0) && ::GetLastError() != ERROR_IO_PENDING)
            {
                ...
                if (errorCode != WSAENOTSOCK && errorCode != WSAECONNRESET && errorCode != WSAECONNABORTED)
                {
                    g_winlog.AddLog(LOG_ERROR, "CIOSocket::Write %x(%x) err=%d", m_hSocket, this, errorCode);
                }
