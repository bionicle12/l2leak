...
#include "l2auth/model/ServerPacketList.h"
...
void ServerKind::LoadServerList()
{
    ...
    if (m_servers.empty())
    {
        ...
        if (sql.Execute("Select Max(kind) From server With (Nolock)"))
        {
            if (sql.Fetch(&notFound))
            {
                if (notFound)
                {
                    ...
                    sql.ResetHtmt();
                    g_winlog.AddLog(LOG_ERROR, "Load ServerKindList fail!");
                    return;
                }
                ...
                for (int frameType = FrameType_DefaultOrCompressed; frameType < FrameType_Compressed; ++frameType)
                {
                    ...
                    if (m_serverFrames[frameType] == NULL)
                    {
                        g_winlog.AddLog(LOG_ERROR, "Load ServerKindList Memory Allocation Fail");
                        success = false;
                        break;
                    }
        ...
        {
            g_winlog.AddLog(LOG_ERROR, "Loading CServerList Fail.");
            success = false;
        }
    }
...
void ServerKind::ReloadServerList()
{
    if (m_servers.empty())
    {
        g_winlog.AddLog(LOG_ERROR, "Reload ServerKindList Fail!");
        return;
    }
            ...
            else
            {
                ...
                if (++serverKindsCount > m_serverKindsCount)
                {
                    ...
                    for (int frameType = FrameType_DefaultOrCompressed; frameType < FrameType_Compressed; ++frameType)
                    {
                        ...
                        {
                            g_winlog.AddLog(LOG_ERROR, "Reload ServerKindList Fail!, Memory Allocation Fail");
                            return;
                        }
                                ...
                                "server Order by id"))
                {
                    ...
                    while (notFound == false)
                    {
                        ...
                        {
                            g_winlog.AddLog(LOG_ERROR, "Reloading serverlist fail, serverid is not serialized");
                            return;
                        }
                ...
                {
                    g_winlog.AddLog(LOG_ERROR, "Reload CServerList Fail.");
                }
            }
        }
        else
...
void ServerKind::MakeServerListFrame()
{
    ...
    if (!success)
    {
        g_winlog.AddLog(LOG_ERROR, "ReLoadServerListFrame Fail!!");
    }
}
