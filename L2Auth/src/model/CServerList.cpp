...
CServerList::~CServerList()
{
    for (int i = 1; i < 10; ++i)  // FIXME: 0 is not deleted?
    {
    ...
    if (g_Config.readLocalServerList)
    {
        ...
        if (file)
        {
            ...
            while ((str != NULL) || !feof(file))
            {
                if (strlen(buffer) <= 10)
                {
                    g_winlog.AddLog(LOG_ERROR, "can't load serverlist file");
                    return;
                }
    ...
    else
    {
        ...
        if (m_servers.empty())
        {
                            ...
                            "server Order by id"))
            {
                ...
                while (notFound == false)
                {
                    ...
                    {
                        g_winlog.AddLog(LOG_ERROR, "loading serverlist fail, serverid is not serialized");
        ...
        else
        {
                            ...
                            "server Order by id"))
            {
                ...
                if (m_tmpServer.serverId < currentServersSize)  // remove missed servers. FIXME:
...
#if defined _WIN64
                ::InterlockedExchange64((volatile LONGLONG*)&m_defaultOrCompressedSize, (LONGLONG)defaultOrCompressedSize);
#else
                ::InterlockedExchange((volatile LONG*)&m_defaultOrCompressedSize, (LONG)defaultOrCompressedSize);
#endif
                memcpy(m_compressedFrameBuffers, &m_serverCount, 1u);
            }
        }
        if (m_servers.empty())
        {
            g_winlog.AddLog(LOG_ERROR, "Server load fail, please check server table or DB connection string");
        }
    ...
    for (int i = 1; i <= m_serverNumber; ++i)
    {
        ...
        *iter++ = worldServer.serverId;
        ...
        if (strlen(worldServer.serverName) >= 20)
        {
            ...
            serverName[19] = 0;
            // swprintf((wchar_t*)iter, L"%S", serverName); FIXED
            swprintf((wchar_t*)iter, sizeof(buffer) / sizeof(*buffer), L"%S", serverName);
        }
        else
        {
            // swprintf((wchar_t*)iter, L"%S", worldServer.serverName); FIXED
            swprintf((wchar_t*)iter, sizeof(buffer) / sizeof(*buffer), L"%S", worldServer.serverName);
        }
        iter += 40;
