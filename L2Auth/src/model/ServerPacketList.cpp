...
void ServerPacketList::MakeServerKind2(const WorldServer& worldServer)
{
    ...
    if (strlen(worldServer.serverName) >= 20)
    {
        ...
        serverName[19] = 0;
        // swprintf((wchar_t*)&m_buff[index], L"%S", serverName); FIXED
        swprintf((wchar_t*)&m_buff[index], sizeof(m_buff) / sizeof(*m_buff), L"%S", serverName);
    }
    else
    {
        // swprintf((wchar_t*)&m_buff[index], L"%S", worldServer.serverName); FIXED
        swprintf((wchar_t*)&m_buff[index], sizeof(m_buff) / sizeof(*m_buff), L"%S", worldServer.serverName);
    }
