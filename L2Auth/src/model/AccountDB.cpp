...
bool AccountDB::SendWantedServerLogout(const char* accName, int uid, int serverId)
{
    if (g_Config.useWantedSystem && !CWantedSocket::isReconnecting && serverId > 0)
    {
        ...
        if (g_Config.gameID == 8)
        {
            ...
            {
                g_winlog.AddLog(LOG_NORMAL, "Wanted User LogOut, %s", accName);
...
bool AccountDB::RegAccount(const LoginUser& user, int uid, CAuthSocket* gameClient, int totalTime, int zero)
{
    ...
    if (success)
    {
        if (user.notifyGameServer & 0x10)
        {
            ...
            success = WorldSrvServer::SendSocket(worldServer.ipAddress, "cdsdddd", AuthToServerPacket_AboutToPlayRequest, uid, user.accountName, totalTime, user.loginFlag, user.warnFlag, user.payStat);
            if (!success)
            {
                m_spinLock.Enter();
                UserMap::iterator it = m_accounts.find(uid);
                ...
                m_spinLock.Leave();
                gameClient->Send("cc", AuthToClientPacket_LoginFail, REASON_IP_SESSION_REJECTED);
            }
        }
        if (success)
        {
            ...
            gameClient->Send("cddddddddd", AuthToClientPacket_LoginOk, uid, sessionKey, AccountDB::updateKey, AccountDB::updateKey2, user.payStat, totalTime, zero, user.warnFlag, user.loginFlag);
        }
    }
    else
    {
        KickAccount(uid, REASON_ACCOUNT_IN_USE, true);
        gameClient->Send("cc", AuthToClientPacket_LoginFail, REASON_ACCOUNT_IN_USE);
    }
...
bool AccountDB::KickAccount(int uid, char kickReason, bool sendToClient)
{
    ...
    {
        Utils::WriteLogD(LogId_Leaveworld2, accountName, clientIP, payStat, age, sexAndCentury, 0, 0, uid);
    }
    ...
    if (loginState != LoginState_LoggedToGS && loginState != LoginState_AcceptedByGS)
    {
        ...
        {
            CAuthSocket::Send(gameSocket, "cc", AuthToClientPacket_KickedFromGS, kickReason);
        }
        return found;
    }
    ...
    if (loggedServerId <= g_serverList.m_serverNumber && loggedServerId >= 1)
    {
        ...
        WorldSrvServer::SendSocket(worldServer.ipAddress, "cdcs", AuthToServerPacket_KickAccountRequest, uid, kickReason, accountName);
        if (payStat < 1000 && payStat > 0)
        {
            int sessionKey = g_IPSessionDB.DelSessionID(uid);
            if (sessionKey > 0)
...
{
    g_accountDb.TimerCallback(reinterpret_cast<int>(uid));
}
...
void AccountDB::TimerCallback(int uid)
{
    ...
    if (loginState == LoginState_AcceptedByGS && removed)  // 5 min timeout
    {
        if (selectedGServerId <= g_serverList.m_serverNumber && selectedGServerId >= 1)
        {
            ...
            WorldSrvServer::SendSocket(worldServer.ipAddress, "cdcs", AuthToServerPacket_KickAccountRequest, uid, 0, accountName);
        }
...
void AccountDB::RemoveAll()
{
    ...
    for (UserMap::iterator it = m_accounts.begin(); it != m_accounts.end(); it = m_accounts.erase(it))
    {
        ...
        {
            CAuthSocket::Send(user.gameSocket, "cc", AuthToClientPacket_PlayFail, PLAY_FAIL_GS_ERROR);
        }
...
SOCKET AccountDB::FindSocket(int uid, bool recreate)
{
    ...
    if (recreate)
    {
        intptr_t wrappedUid = uid;
        ::CreateTimerQueueTimer(&phNewTimer, 0, &AccountDB::TimerRoutine, reinterpret_cast<PVOID>(wrappedUid), 300000u, 0, 0);
    }
...
bool AccountDB::removeAccountPreLogIn(int uid, SOCKET socket)
{
    ...
    if (success)
    {
        ...
        {
            Utils::WriteLogD(LogId_Leaveworld2, accName, clientIP, payStat, age, sexAndCentury, 0, 0, uid);
        }
    }
...
bool AccountDB::recordGamePlayTime(int uid, int serverId)
{
    ...
    if (success)
    {
        ...
        Utils::WriteLogD(LogId_Login, accName, connectedIP, payStat, age, sexAndCentury, 0, g_reporter.loggedUsers, uid);
    }
...
bool AccountDB::quitGamePlay(int uid, int serverId)
{
    ...
    {
        intptr_t wrappedUid = uid;
        ::CreateTimerQueueTimer(&phNewTimer, NULL, &AccountDB::TimerRoutine, reinterpret_cast<PVOID>(wrappedUid), g_Config.socketTimeOut, 0, 0);
    }
...
PlayFail AccountDB::AboutToPlay(int uid, char* accName, int totalTime, int loginFlag, int warnFlag, int sessionKey, CAuthSocket* gameClient, signed int serverId, int payStat)
{
    ...
    {
        gameClient->Send("cc", AuthToClientPacket_PlayFail, PLAY_FAIL_GS_ERROR);
    ...
    if (g_Config.userData)
    {
        ...
        if (serverId <= g_serverList.m_serverNumber && serverId >= 1)
        {
            ...
            success = WorldSrvServer::SendSocket(worldServer.ipAddress, "cdsdddbd", AuthToServerPacket_AboutToPlayRequest, uid, accName, totalTime, loginFlag, warnFlag, sizeof(userData), userData, payStat);
        }
    }
    else if (serverId <= g_serverList.m_serverNumber && serverId >= 1)
    {
        WorldServer worldServer = g_serverList.GetAt(serverId);
        success = WorldSrvServer::SendSocket(worldServer.ipAddress, "cdsdddd", AuthToServerPacket_AboutToPlayRequest, uid, accName, totalTime, loginFlag, warnFlag, payStat);
    }
    ...
    if (gameClient != NULL)
    {
        if (success)
        {
            ...
            {
                gameClient->Send("cc", AuthToClientPacket_PlayFail, errorCode);
                return errorCode;
            }
        }
        else
...
// L2AuthD 0x0040F5B0
size_t AccountDB::GetUserNum() const
{
    m_spinLock.Enter();
...
LoginFailReason AccountDB::UserTimeLogin(unsigned int uid, LoginUser* /*userInfo*/, int* userTime)
{
    ...
    {
        errorCode = REASON_NO_TIME_LEFT;  // FIXME: buggy, getUserGameTime() might return other reason
    }
    return errorCode;
}
...
bool AccountDB::RecordLogout(int uid, time_t loginTime, int lastWorld, in_addr userIp, int gameType, char* accountName, int payStat, int birthdayEncoded, int restOfSsn, SexAndCentury sexAndCentury, int age, int someClientCookie)
{
    ...
    Utils::WriteLogD(LogId_Logout, accountName, userIp, payStat, age, sexAndCentury, 0, usedTime, uid);
    ...
    if (payStat >= 1000 || payStat <= 0)
    {
        ...
        {
            CDBConn sql(&g_linDB);
            size_t accLength = ::strlen(accountName);
            SQLLEN len1 = 0;
