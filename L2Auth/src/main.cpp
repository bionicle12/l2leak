...
#include "l2auth/crypt/DesNewCrypt.h"
#include "l2auth/crypt/PwdCrypt.h"
...
int __stdcall WinMain(HINSTANCE instance, HINSTANCE /*hPrevInstance*/, LPSTR /*lpCmdLine*/, int nShowCmd)  // L2AuthD 0x0042C965
{
    HWND mainWndHandler = ::FindWindowA(0, "AuthServer");  // FIXME: add build version, and then find
    if (mainWndHandler)
    {
        ::MessageBoxA(0, "냦냭", "샌만 샎쇵벭맶낡 뷇쟠뗇뻮 샖뷀듏듙.", MB_ICONSTOP);
        exit(0);
    }
    ...
    wndClass.hIcon = 0;  //::LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON_L2AUTH));
    ...
    {
        g_winlog.AddLog(LOG_ERROR, "WSAStartup error 0x%x", wsaStartupResult);
        return 0;
    }
    ...
    MainWindow::logWnd = ::CreateWindowExA(
        WS_EX_CLIENTEDGE,  // L2AuthD 0x200u,
        (LPCSTR)className,
        "",
        WS_CHILD,  // L2AuthD 0x40000000u,
        0,
        30,
        640,
        720,
    ...
    if (strlen(g_Config.logDirectory))
    {
        ...
        g_winlog.AddLog(LOG_NORMAL, "LOADED Config");
        g_winlog.AddLog(LOG_INF, "BuildNumber : %d", g_buildNumber);

        if (g_Config.acceptCallNum == 0)
        {
            g_winlog.AddLog(LOG_ERROR, "AcceptCallNull 이 0 이면 어떤 클라이언트도 접속이 안됩니다. 1 로 자동세팅합니다.");
            g_Config.acceptCallNum = 1;
        }

        if (g_Config.socketTimeOut == 0)
        {
            g_winlog.AddLog(LOG_ERROR, "SocketTimeOut이 0이면 Connection이 바로 끊기게 됩니다. 180으로 자동세팅합니다.");
        ...
        if ((g_Config.mailServer != NULL) && (::strlen(g_Config.mailServer) != 0))
        {
            ...
            {
                g_winlog.AddLog(LOG_ERROR, "Sendmail initialize Fail");
            }
        }
        ...
        if (g_Config.useIPServer)
        {
            ...
            {
                g_IPSocket->Initialize(IOCP::g_hCompletionPort);
            }
        }
        ...
        if (g_Config.useWantedSystem)
        {
            ...
            {
                g_SocketWanted->Initialize(IOCP::g_hCompletionPortExtra);
            }
        }
    ...
    else
    {
        ...
        g_actionLog.SetDirectory(g_Config.logDirectory);
        g_errLog.SetDirectory(g_Config.logDirectory);
        g_winlog.AddLog(LOG_ERROR, "Error load config.txt");
        g_logdfilelog.SetDirectory(g_Config.logDirectory);
    }
